...
for ($i = 1; $i <= $loop; $i++) {
	...
	if ($firstGuess) {
		...
		echo "the first guess is: " . $guess . "\n";

	}
	else {

		echo "executing guess number $i" . "\n";
		$guess = getGoodGuess();
	}
	...
	if ($numberLettersCorrect > $bestNumCorrect) {
		$bestNumCorrect = $numberLettersCorrect;
		$bestGuess = $guess;
	}

	$data[$obj->token]["numberLettersCorrect"] = $numberLettersCorrect;
	$data[$obj->token]["guess"] = $guess;
...
function generateGuess() {
	...
	for ($j = 0; $j < $secretLength; $j++) {
		$thisGuess[$j] = $alphabet[rand(0, 25)];
	}
	return join("", $thisGuess);
}

function getGoodGuess() {
	global $guesses;
	...
	while ($badGuess) {
		...
		if (in_array($guess, $guesses)) { $break; }
